...
default_dependency.specialists.Detective       → agent_detective()
...
default_dependency.bait_tester      → inspect_external_content()
```
...
| `inspect_content(text)`                 | A connected adapter before consuming external text | Calls the configured Bait Tester with text and signal only.                                                       |
...
| First parallel group       | Detective resolves the requested brand or product; unselected Vault Keeper returns no update.         |
...
| Final result               | Returns `needs_review`, empty public analysis fields, and limitations naming the missing review work. |

The internal result map contains the five selected names. It is not exposed directly in the API response.

If the prompt also asks about boycott or ownership, Dispatcher should select Investigator, add the `ethics` check, and make Skeptic wait for Investigator. The connected Investigator receives Detective's validated subject, charges both evidence lookups to the shared tool budget, and sends Boycat content through Bait Tester. It does not re-extract an identity from the raw prompt in this path.
...
Bait Tester is not a scheduled member of Dispatcher's `selected_agents`. Its callback receives no user ID, dependency map, or tool helper from this executor. The connected agent has no tools or memory and returns only a security decision; approved content is passed through unchanged. Its output still needs later evidence review.
