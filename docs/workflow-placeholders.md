...
Detective validates its structured identity result. An identified product or brand completes its step; selection requirements, no match, and provider unavailability return `needs_input` with distinct limitations. `selection.options` contains user choices only, while `related_products` contains a bounded product page with total and pagination metadata. It retains the lookup agent's Groq configuration and read-only tools. The workflow supplies its cancellation signal and wraps each lookup tool call in the shared tool budget. Successful Open Food Facts results pass through the connected, isolated Bait Tester before their text reaches the model.

Investigator runs only when Dispatcher selects it and Detective completed with one identified subject. A brand subject supplies its canonical name; a product subject supplies the canonical brand plus a bounded, deduplicated list of provider brand labels. The product name remains context and is never used as a substitute for a missing brand. Local knowledge checks every candidate, Boycat checks each distinct bounded identity, and conflicting verified identities require review. These calls share the workflow tool budget, Boycat content passes through Bait Tester, and the optional model-written draft is disabled in the workflow. Ambiguous or unavailable evidence maps to `needs_review`; an unresolved Detective identity prevents Investigator from running.
