...
describe('Consumer workflow startup', () => {
  ...
  it('connects the default Detective adapter to product-brand lookup and preserves unresolved gates', async () => {
    ...
    try {
      const cases = [
        { query_type: 'product', found: true, status: 'needs_review' },
        { query_type: 'brand', found: true, status: 'needs_review' },
        { query_type: 'product', found: false, status: 'needs_input' },
        { query_type: 'unknown', found: true, status: 'needs_input' },
      ] as const
      for (const { query_type, found, status } of cases) {
        lookup.mockResolvedValue({ output: { query_type, found, message: 'Unreviewed lookup details' } } as Awaited<
